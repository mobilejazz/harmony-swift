...
public extension Dictionary {
    ...
    /// - Throws: The first mapping thrown error
    func mapKeys<T: Hashable>(_ executor: Executor, _ transform: @escaping (Key) throws -> T) rethrows -> [T: Value] {
    ...
    func mapValues<T>(_ executor: Executor, _ transform: @escaping (Value) throws -> T) rethrows -> [Key: T] {
        return try self.map(executor) { element -> T in
    ...
    /// - Returns: A dictionary with mapped values
    /// - Throws: The first mapping thrown error
    func map<T>(_ executor: Executor, _ transform: @escaping ((key: Key, value: Value)) throws -> T) rethrows -> [Key: T] {
