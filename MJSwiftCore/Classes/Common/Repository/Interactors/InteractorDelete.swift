...
extension Interactor {
    ...
    public class DeleteByQuery <T> : QueryInteractor <T> {
        public func execute(object: T? = nil, query: Query = BlankQuery(), operation: Operation = .none, in executor: Executor? = nil) -> Future<Bool> {
        ...
        public func execute<K>(object: T? = nil, forId id: K, operation: Operation = .none, in executor: Executor? = nil) -> Future<Bool> where K:Hashable {
            let executor = executor ?? self.executor
            return executor.submit { resolver in
    ...
    public class Delete <T> : DirectInteractor <T> {
        public func execute(object: T? = nil, operation: Operation = .none, in executor: Executor? = nil) -> Future<Bool> {
            let executor = executor ?? self.executor
            return executor.submit { resolver in
    ...
    public class DeleteAllByQuery <T> : QueryInteractor <T> {
        public func execute(objects: [T] = [], query: Query = BlankQuery(), operation: Operation = .none, in executor: Executor? = nil) -> Future<Bool> {
            let executor = executor ?? self.executor
            return executor.submit { resolver in
        ...
        public func execute<K>(objects: [T] = [], forId id: K, operation: Operation = .none, in executor: Executor? = nil) -> Future<Bool> where K:Hashable {
            let executor = executor ?? self.executor
            return executor.submit { resolver in
    ...
    public class DeleteAll <T> : DirectInteractor <T> {
        public func execute(objects: [T] = [], operation: Operation = .none, in executor: Executor? = nil) -> Future<Bool> {
            let executor = executor ?? self.executor
            return executor.submit { resolver in
